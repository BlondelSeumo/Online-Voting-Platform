 ...
 if(Token::check(Input::get('token'))){
	...
	$validation = $validator->check($_POST, [
	  ...
	  if (!$validation->fails()) {
			...
			$qs = DB::getInstance()->get("admin", "*", ["email" => Input::get('email')]);
			if ($qs->count()) {
	         	foreach($qs->results() as $rs) {
	            }
				
				$token = Token::generate(); 
				$Update = DB::getInstance()->update('admin',[
				    'tokencode' => $token
				],[
				    'adminid' => $rs->adminid
				  ]);
				
			    $id = base64_encode($rs->adminid);
				...
				$message= "
					   ...
					   <br />
					   <a href='". $web->url ."admin/reset/$id/$token'>click here to reset your password</a>
					   <br /><br />
					   thank you :)
					   ";
            ...
            <div class="form-body">          	
			 ...
			 <div class="form-foot">
			  <div class="form-actions">
			   ...
			   <input type="submit" name="register" value="<?php echo $lang->forgot; ?> <?php echo $lang->password; ?>" class="kafe-btn kafe-btn-default full-width"><br></br>
			  <a href="<?php echo $web->url; ?>admin/login" class="kafe-btn kafe-btn-danger full-width"><?php echo $lang->login; ?></a>
			  </div><!-- /.form-actions -->
			 </div><!-- /.form-foot -->
		   </form>
