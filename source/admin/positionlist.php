...
<body class="skin-green sidebar-mini">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><?php echo $lang->position; ?> <?php echo $lang->list; ?></li>
          </ol>
        </section>
		 ...
		 <div class="row">	
		 	<div class="col-md-12">
                ...
                <div class="box-header">
                  <h3 class="box-title"><?php echo $lang->position; ?> <?php echo $lang->list; ?></h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                 <div class="table-responsive">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
				    <?php
                     $query = DB::getInstance()->get("position", "*", ["ORDER" => "date_added DESC"]);
                     if($query->count()) {
						foreach($query->results() as $row) {
							
                     $q1 = DB::getInstance()->get("organization", "*", ["organizationid" => $row->organizationid ]);
                     if($q1->count()) {
						foreach($q1->results() as $r1) {
						}
					}		
										
					    echo '<tr id="tr'.$row->positionid.'">';
					    echo '<td>'. escape($row->name) .'</td>';
					    echo '<td>'. escape($r1->name) .'</td>';
					    echo '<td>
					    
					      <a href="'. $web->url .'admin/editposition/' . escape($row->positionid) . '" class="btn btn-success btn-xs" data-toggle="tooltip" title="' . $lang->edit . '"><span class="fa fa-edit"></span></a>
					      '.getDeletePosition($row->positionid) .'
					      </td>';
					    echo '</tr>';
					   }
					}else {
    ...
    <script type="text/javascript">	
	function delete_the_position(id) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo $web->url; ?>source/admin/template/requests/deleteposition.php",
			data: "id="+id, 
			cache: false,
